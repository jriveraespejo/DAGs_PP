...
%
\begin{frame}
	{Similar scenario\footnote{\citet{Cinelli_et_al_2021} (p. 3)}}
	%
	\begin{columns}
		%
		\begin{column}{0.5\textwidth}
		...
		\begin{column}{0.5\textwidth}  
			...
			\begin{figure}
				...
				\begin{tikzpicture}
					% nodes
					\node[formula] at (-2,0.8) {$U_{M}$};
					\node[formula] at (-1,1) {$M$};
					\node[formula] at (-2,0) {$U_{N}$};
					\node[formula] at (-1,-0.3) {$N$};
					...
					% paths
					\draw [{Circle [open]}-{latex}](-1.7,0.75)--(-1,0.75); % Um->M
					\draw [{Circle[color=red]}-{latex}](-0.95,0.8)--(-0.95,0.05); % M->N
					\draw [-{latex}](0,0.75)--(-0.9,0.75); % Ux->M
					\draw [{Circle [open]}-{latex}{Circle}](-1.7,0)--(-0.9,0); % Un->N
					\draw [-{latex}](-0.9,0)--(0.9,0); % N->K
					\draw [{Circle [open]}-{latex}{Circle}](1.7,0)--(0.9,0); % Uk->K
					%\draw [-{latex}](-0.95,0.05)--(0,0.75); % N->M
					\draw [{Circle [open]}-{latex}](0,0.8)--(0.9,0.1); % M->K
...
%
%
\begin{lhframe}[rhgraphic={\includegraphics[scale=0.7]{fork5_reg3.png}}]
	{did it worked???}
