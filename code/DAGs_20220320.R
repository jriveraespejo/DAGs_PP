...
f_plot1(dsim=dsim, ipar='M', n=100, xR=c(-2.5,1.5), by=0.5, 
        leg=F)
par(mfrow=c(1,1))
# dev.off()
...
# rep = to use in replication
#
f_sim = function(n=100, bU=0.5, bUX=0.8, bUY=0.7, bXY=0, bZ=0, rep=F){
  
  # # test
  # n=100; bU=0.5; bUX=0.8; bUY=0.7; bXY=0; rep=F
  
  # sim
  Ux = sample(1:5, n, replace=T)
  Uy = sample(1:5, n, replace=T)
  Z = rnorm(n, bU*Ux + bU*Uy)
  X = rnorm(n, bUX*Ux + bZ*Z)
  Y = rnorm(n, bUX*Uy + bXY*X + bZ*Z)
  d = data.frame(Ux,Uy,Z,X,Y)
...
# relationships
d = f_sim(n=100, bU=0.5, bUX=0.8, bUY=0.7, bXY=0, bZ=0, rep=F)
psych::pairs.panels(d)
...
# sampling variation
par(mfrow=c(2,1))
dsim = replicate( 1e4, f_sim(n=20, bU=0.5, bUX=0.8, bUY=0.7, bXY=0, bZ=0, rep=T) )
f_plot1(dsim=dsim, ipar='X', xR=c(-1.5,1), by=0.1)

dsim = replicate( 1e4, f_sim(n=100, bU=0.5, bUX=0.8, bUY=0.7, bXY=0, bZ=0, rep=T) )
f_plot1(dsim=dsim, ipar='X', xR=c(-1.5,1), by=0.1)
par(mfrow=c(1,1))
...
# (pipe bias: contextual confounds) ####
#
...
f_plot1(dsim=dsim, ipar='WR', xR=c(-2.5,2), by=0.5)
f_plot1(dsim=dsim, ipar='WP', xR=c(0,3), by=0.5)
...
f_plot1(dsim=dsim, ipar='E', xR=c(-2,1), by=0.5)
...
# just answers a diffrente research question
...
# more specifically the shown difference is G -> A | D==A
...
# replicate
#
f_sim = function(d=UCBadmit){
  
  # gender controlled by department
  m_res = glm(cbind(admit, applications) ~ applicant.gender + dept, 
              data=d, family='binomial') 
  b1 = coef(m_res)['applicant.gendermale'] 
  b1 = c(b1, inv_logit(b1) ) # probability
  
  # gender NOT controlled by department
  m_res = glm(cbind(admit, applications) ~ applicant.gender, 
              data=d, family='binomial')
  b2 = coef(m_res)['applicant.gendermale'] 
  b2 = c(b2, inv_logit(b2) ) # probability
  
  # return object
  b = c(b1, b2)
  names(b) = c('GR','GP','GRm','GPm')
  return( b )
  
}

# parameter variation
dsim = replicate( 1e4, f_sim() )

par(mfrow=c(2,1))
f_plot1(dsim=dsim, ipar='GR', xR=c(-0.1,0.5), by=0.1)
f_plot1(dsim=dsim, ipar='GP', xR=c(0.3,0.8), by=0.1)
par(mfrow=c(2,1))
# G -> A, if not controlled by D
# notice the difference between the relative and absolute effects
